...
def read_station(file, station_crop):
    ...
    df = df[(df['latitude'] >= station_crop[2]) &
            (df['latitude'] <= station_crop[3])]
    df = df[(df['longitude'] >= station_crop[0]) &
            (df['longitude'] <= station_crop[1])]
...
def plot_lines(emc_dict, wrf_dict, wrf, stations,
               type='mean', loc=True):
    '''
    Plot the lines
    '''
    ...
    if loc:
        ...
        if type == 'mean':
            axs[0].scatter(stations_lon, stations_lat,
                           edgecolors='red9', facecolors="None")
        elif type == 'station':
            clist = []
...
def main():
    ...
    station_locs = {}
    for vname in vnames:
        station_locs[vname] = emc_dict[vname]\
    ...
    # plot comparisons
    f = plot_lines(emc_dict, wrf_dict, wrf, stations,
                   type=plot_type, loc=plot_locs)
